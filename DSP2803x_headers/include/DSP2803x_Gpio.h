...
//----------------------------------------------------
// GPIO A 控制寄存器位定义 */
struct GPACTRL_BITS {        // 位   描述
   Uint16 QUALPRD0:8;        // 7:0    去抖周期
   Uint16 QUALPRD1:8;        // 15:8   去抖周期
   Uint16 QUALPRD2:8;        // 23:16  去抖周期
   Uint16 QUALPRD3:8;        // 31:24  去抖周期
};

union GPACTRL_REG {
   Uint32              all;  // 整个寄存器的 32 位值
   struct GPACTRL_BITS bit;  // 位域结构体，用于逐位访问
};

//----------------------------------------------------
// GPIO B 控制寄存器位定义 */
struct GPBCTRL_BITS {        // 位   描述
   Uint16 QUALPRD0:8;        // 7:0    去抖周期
   Uint16 rsvd1:8;           // 15:8   保留位
   Uint16 rsvd2:16;          // 31:16  保留位
};
