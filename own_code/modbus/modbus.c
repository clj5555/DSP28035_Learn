 ...
 * @return 计算得到的CRC校验值
 *
 * 本函数通过循环计算输入字符串的CRC校验值使用了两个CRC表(auchCRCHi和auchCRCLo)
 * 分别更新CRC的高位和低位字节在每次循环中根据当前字符和CRC的高位字节的异或结果
 * 查找CRC表中的对应值来更新CRC的高位和低位字节最终将CRC的高位和低位字节组合成
 */
...
static Uint16 get_crc16(Uint16 *pbuff, Uint16 len)
{
    ...
    // 检查长度，如果为0则默认长度为1，确保至少有一个字节的数据进行CRC计算
    if (len == 0)
    {
        len = 1;
    }
    // 遍历缓冲区中的每个字节，进行CRC16校验码的计算
    while (len--)
    {
        crc ^= *pbuff;
        // 对每个字节的8位进行位操作，计算CRC16校验码
        for (i = 0; i<8; i++)
        {
            // 如果当前CRC值的最低位为1，则进行右移和异或操作
            if (crc & 1)
            {
                crc >>= 1;
                crc ^= 0xA001;
            ...
            {
                // 如果当前CRC值的最低位为0，则仅进行右移操作
                crc >>= 1;
            }
        }
        pbuff++;
    }
    // 调整CRC16校验码的字节顺序，以符合CRC16算法的常规输出格式
    ret1 = crc >> 8;
    ret1 = ret1 | (crc << 8);
    return(ret1);
}
...
{
    static unsigned int startaddr,number,code;
...
//              MODBUS_03_Return(COM1_TxBuff,number);
            ...
            case 0x06:
                ...
                if(((buf[2]<<8|buf[3])== 0x0001)&&((buf[4]<<8|buf[5]) == 0x0100))
                {
             //       LED_ON;
            //                  Prints("I'm here:1\r\n");
                }
                if(((buf[2]<<8|buf[3])== 0x0001)&&(buf[4] == 0x06))
                {
              //      LED_ON;
                    Delay_nMS((Uint16)buf[5]*10);
              //      LED_OFF;
                    Delay_nMS(100);
               //     LED_ON;
                    Delay_nMS(100);
            ...
            //                  Prints("I'm here:2\r\n");
                ...
                if(((buf[2]<<8|buf[3])== 0x0001)&&((buf[4]<<8|buf[5]) == 0x0200))
                {
            //        LED_OFF;
            //                  Prints("I'm here:3\r\n");
                }
            ...
            case 0x10:
                ...
                if(((buf[2]<<8|buf[3])== 0x0001)&&((buf[4]<<8|buf[5]) == 0x0100))   //01 06 00 01 01 00 D9 9A
                {
             //       LED_ON;
            //                  Prints("I'm here:1\r\n");
                }
                if(((buf[2]<<8|buf[3])== 0x0001)&&(buf[4] == 0x06))                 //01 06 00 01 06 0A 5B AD 寤舵椂10
                {
            //        LED_ON;
                    Delay_nMS((Uint16)buf[5]*10);
             //       LED_OFF;
            ...
            //                  Prints("I'm here:2\r\n");
                ...
                if(((buf[2]<<8|buf[3])== 0x0001)&&((buf[4]<<8|buf[5]) == 0x0200))   //01 06 00 01 02 00 D9 6A                                                                                           //01 06 00 01 02 00 D9 6A
                {
            //        LED_OFF;
            //                  Prints("I'm here:3\r\n");
                }
...
//  Prints("\r\n   \r\n");
    ...
    // 检查计算出的结束地址是否超出允许范围
    if((reg_start_addr + bit_num) > REG_LAST_ADDR)
    {
        COM1_TxBuff[0] = COM1_RxBuff[0];   // 如果超出范围，设置响应缓冲区的ID

        COM1_TxBuff[1] = COM1_RxBuff[1];   // 设置响应缓冲区的第二个字节

        COM1_TxBuff[2] = 0x02;             // 设置错误代码

        MODBUS_Send(COM1_TxBuff, 3);       // 发送错误响应
        // 返回函数
        return;
    }

    COM1_TxBuff[0] = COM1_RxBuff[0];      // 设置响应缓冲区的ID

    COM1_TxBuff[1] = COM1_RxBuff[1];      //// 设置响应缓冲区的第二个字节

    COM1_TxBuff[2] = bit_num;           //// 设置响应缓冲区中的位数量
...
//  {
//      COM1_TxBuff[3 + i] = ( Gu16_modbus_bits_outputIO[reg_start_addr + i]);//256--宸︾Щ8浣?
...
static void modbus_03_solve(void)
{
    ...
    if((reg_start_addr + reg_num) > REG_LAST_ADDR)
    {
    ...
    {
        COM1_TxBuff[3 + i*2] = ( Gu32_modbus_outputIO[reg_start_addr + i] / 65536 );//256--宸︾Щ8浣?
        COM1_TxBuff[4 + i*2] = ( Gu32_modbus_outputIO[reg_start_addr + i] % 65536 );
    }
...
static void modbus_05_solve(void)
{
    ...
    Gu32_modbus_outputIO[reg_start_addr]=((Uint32)COM1_RxBuff[4]<<16)|COM1_RxBuff[5];//楂樺瓧鑺傚湪鍓? Gu32_modbus_outputIO[0x0001]=ff00
//  Gu32_modbus_outputIO[reg_start_addr]|=((Uint32)COM1_RxBuff[5]);
...
//  COM1_TxBuff[4] = ( Gu32_modbus_outputIO[reg_start_addr] / 65536 );
...
static void modbus_06_solve(void)
{
    ...
    reg_start_addr = ((Uint32)COM1_RxBuff[2]<<16)|COM1_RxBuff[3];
    Gu32_modbus_outputIO[reg_start_addr]=((Uint32)COM1_RxBuff[4]<<16)|COM1_RxBuff[5];//楂樺瓧鑺傚湪鍓?
//  Gu32_modbus_outputIO[reg_start_addr]|=((Uint32)COM1_RxBuff[5]);//浣庡瓧鑺傚湪鍚?
    ...
    COM1_TxBuff[4]=COM1_RxBuff[4];
    COM1_TxBuff[5]=COM1_RxBuff[5];
    COM1_TxBuff[4] = ( Gu32_modbus_outputIO[reg_start_addr] / 65536 );//256--宸︾Щ8浣?
    COM1_TxBuff[5] = ( Gu32_modbus_outputIO[reg_start_addr] % 65536 );
